...
= Introduction
  ...
  The dataset used for this project is titled "Average Monthly Temperature by US State," compiled by the National Centers for Environmental Information. The link for this dataset is: https://www.kaggle.com/datasets/justinrwong/average-monthly-temperature-by-us-state. It contains one CSV file that has average temperatures for 48 states (excluding Hawaii and Alaska) by month. Due to the extensive dates covered, starting from 1950 to 2022, the data allows for analysis of climate change within the United States. To limit the scope of this project, four states in four distinct geographical regions in the United States were chosen tp analyze: California, Illinois, New York, and Texas. The temperature was measured in degrees Fahrenheit and the centroid latitude and longitude were also provided for each state. 

= Methodology

The National Centers for Environmental Information's dataset used in this study was first parced to only include the four states under analysis then was cleaned of extraneous datapoints and variables. A LSTM machine learning model was chosen to forecast future monthly average temepratures for each of the four afrementioned states. This particular type of model was chosen for its strength when working with sequential data and forecasting. Exogenous variables within the system include previous temperature data, a cummulative monthly mean temperature from 1950-2000 for each state, and state identity. The future monthly temperatures comprise the endogenous variables in the system. 

  Analysis was done in Python using the built-in PyTorch library. The raw dataset was loaded using the Pandas library before it was passed into PyTorch tensors the model could read. Temperature's were normalized using the cummulative mean temperature provided as well as a per month standar deviation value. The LSTM was fed the latest 24 months of temeprature data which composed the input sequence. The forecast length was set to 12 to predict the next year of temeprature data. A two-layer LSTM was used in this study with an input size of 1, a hidden state size of 64, and a dropout rate of 0.3. Training was performed over 15 epochs with the Adam optimzer and a learning rate of 0.004. While no sliding window was included in the model architecture, a sliding window could be employed to generate additional training sequences potentially leading to a more robust model. The PlotLosses function from the livelossplot library was used to capture training and validation loss in real time. Once training was complete, the model was set to its evaluation state and the next 12 months of temeprature data were predicted for each state; predicted temperatures are plotted alongside the previous year's data for a visual comparison of the model's results. 

= Exploratory Data Analysis 

Our dataset provides us with the average temperatures of each state per month by year spanning from January 1950 to August 2022. In this file, we are also provided with the centroid longitude and centroid latitude to ultimately assist with the plotting of a predicted heat model of the United States of America by state. With our data, we want to prove the rate of temperature increase from 1950 to 2022 varies geographically, with higher rates observed in northern and inland states compared to coastal states. This spatial disparity in warming aligns with global research on heatwave distribution, which found that from "2,562 heatwave events identified worldwide, 1,176 occurred over land while only 596 occurred in coastal regions" (Key Laboratory for Semi-Arid Climate Change, School of Atmospheric Sciences, Lanzhou University). The study suggests that inland areas experience more frequent and intense heat events due to weaker oceanic moderation and greater surface heat retention—factors that likely contribute to the accelerated warming trends seen in the temperature change in the interior and northern parts of the United States.
...
NVIDIA has also been developing their weather forecasting technologies. In “Forecasting the Weather Beyond Two Weeks Using NVIDIA Earth-2,” they discuss a deterministic model that focuses on probabilities of seasonal temperature deviations from the norm [1]. Similarly, NVIDIA’s article “Gen AI Super-Resolution Accelerates Weather Prediction with Scalable, Low-Compute Models” demonstrates an animated heat map of the United States temperature predictions at a high resolution [2]. Ideally, our model would have achieved a similar heat map, showing predicted temperatures at the state level. One of the main challenges NVIDIA faces is downscaling coarse resolution data from multiple predictive models to create high-resolution predictions. In contrast, our model, relies only on historical temperature data rather than additional predictive models (e.g. sea surface temperature evolution) to forecast weather. Expanding the dataset to include more than just historical temperatures could improve our future weather predictions, especially when capturing the distinct temperature patterns observed in inland versus coastal states.

== Future work
To compare the LSTM framework with another model, a traditional neural network should also be explored. While a neural network may perform reasonably well for short-term forecasts like 12 months, its accuracy is expected to decline sharply as the forecasting period increases. This limitation becomes more pronounced under climate change, where long-term temperature patterns are increasingly influenced by nonlinear trends due to extreme weather events. In contrast, the LSTM model is better suited for capturing these temporal dynamics and would likely benefit from larger training datasets and extended prediction windows. If time permits, we will explore other alternative predictive models to compare their performance and see if simpler statistical methods or a more advanced deep learning setup can produce more stable and accurate forecasts. This comparison will help determine whether the issues observed in our LSTM model are due to limitations in the model design or the complexity of the climate data. By assessing various modeling approaches using consistent metrics such as MAE and RMSE, we can identify which method best captures the spatial patterns and long-term warming trends in the dataset. This comparative analysis will guide us toward a more reliable forecasting model and strengthen the evidence supporting our conclusions about temperature behavior across the United States.

= References 
[1] Wong, J. (2022, October 8). Average monthly temperature by US state. Kaggle. https://www.kaggle.com/datasets/justinrwong/average-monthly-temperature-by-us-state?resource=download  

[2] Zhang, B., He, Y., & Guan, X. (2025). Increasing trends of land and coastal heatwaves under Global Warming. Atmospheric Research, 318, 108007. https://doi.org/10.1016/j.atmosres.2025.108007 

[3] D. N. Beniaguev, “Forecasting the Weather Beyond Two Weeks Using NVIDIA Earth-2,” NVIDIA Technical Blog, Jul. 2024. Accessed: Nov. 19, 2025. [Online]. Available: https://developer.nvidia.com/blog/forecasting-the-weather-beyond-two-weeks-using-nvidia-earth-2/

[4] L. Ono and S. W. H. Wong, “Gen AI Super-Resolution Accelerates Weather Prediction with Scalable, Low-Compute Models,” NVIDIA Technical Blog, Oct. 2024. Accessed: Nov. 19, 2025. [Online]. Available: https://developer.nvidia.com/blog/gen-ai-super-resolution-accelerates-weather-prediction-with-scalable-low-compute-models/
